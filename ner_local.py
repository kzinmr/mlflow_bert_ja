...
class TokenClassificationDataModule(pl.LightningDataModule):
    ...
    def __init__(self, hparams: Namespace):
        ...
        self.tokenizer_name = os.path.dirname(hparams.model_name_or_path)
        assert os.path.exists(os.path.join(self.tokenizer_name, 'tokenizer.json'))
...
class TokenClassificationModule(pl.LightningModule):
    ...
    def __init__(self, hparams: Union[Dict, Namespace]):
        ...
        tokenizer_name = os.path.dirname(hparams.model_name_or_path)
        assert os.path.exists(os.path.join(tokenizer_name, 'tokenizer.json'))
        self.tokenizer: PreTrainedTokenizerFast = AutoTokenizer.from_pretrained(
            tokenizer_name,
            cache_dir=self.cache_dir,
        )
